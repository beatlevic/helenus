...
Connection.prototype.execute = function(){
  ...
  function onReturn(err, results){
    ...
    callback = NOOP; // prevent the callback from being executed twice if a response comes after the timeout
  }
  
  // set a timeout for executions on the database connection to coop with network partitioning
  timer = setTimeout(function(){
    onReturn({ name: 'TimeoutException', why: 'Execution Timed Out'});
  }, this.timeout);
