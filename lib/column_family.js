 ...
 * @param  {Object}   column   Columns and values to increment by in an object
 * @param  {Object}   options  The thrift options (optional)
 * @param  {Function} callback The callback to call once complete
 */
ColumnFamily.prototype.incr = function (key, columns, options, callback) {
  if (!this.isCounter) throw new Error("Could not increment non CounterColumn columnFamily");
  ...
  for (var column in columns){
    col = new CounterColumn(column, columns[column]);
    mutations.push(new ttype.Mutation({
      column_or_supercolumn: new ttype.ColumnOrSuperColumn({
 ...
 *     <li>buffer: Number of rows to fetch per batch (optional)</li> 
 *     <li>max: Max number of columns to fetch for every row (optional)</li>
 *   </ul>
 * @return {Stream}
 */
ColumnFamily.prototype.getRange = function (options) {
  ...
  keyRangeOptions.count = rowCount ? Math.min(bufferSize, rowCount) : bufferSize;
  ...
  function onComplete(err, val){
    ...
    if (rowCount !== undefined && totalRowCount >= rowCount) {
      ...
      keyRangeOptions.count = (rowCount ? Math.min(bufferSize, rowCount - totalRowCount) : bufferSize) + 1; // add one because the first key is a duplicate
